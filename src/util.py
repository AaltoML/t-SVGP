...
def data_load(name, split=0.8, seed=0, path='../../docs/notebooks/data/', normalize=False):
    """
    ...
    if name == 'boston':
        ...
        Xo = df[['Pregnancies','Glucose','BloodPressure','SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age']].to_numpy()
        ...
        numvars = ['existingchecking', 'duration', 'credithistory', 'purpose', 'creditamount',
         'savings', 'employmentsince', 'installmentrate', 'statussex', 'otherdebtors',
         'residencesince', 'property', 'age', 'otherinstallmentplans', 'housing',
         'existingcredits', 'job', 'peopleliable', 'telephone', 'foreignworker','extra1', 'extra2','extra3','extra4']
        Xo = df[numvars].to_numpy()
        Yo = df['classification'].to_numpy().reshape((-1,1))
    elif name == 'wine':
        Xo =df[['fixed acidity','volatile acidity', 'citric acid',
            'residual sugar', 'chlorides', 'free sulfur dioxide',
            'total sulfur dioxide', 'density', 'pH', 'sulphates','alcohol']].to_numpy()
        Yo = df['y_value'].to_numpy().reshape((-1,1))
    elif name == 'naval':
        Xo = df[['Lever', 'Ship speed', 'Gas Turbine', 'GT rate', 'Gas Generator', 'Starboard Propeller','Port Propeller Torque',
                'Hight Pressure', 'GT Compressor', 'GT Compressor', 'HP Turbine', 'GT Compressor', 'GT Compressor', 'GT exhaust',
                'Turbine','Fuel']].to_numpy()
